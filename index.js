...
const dungeonImages = {
    "금화 저장고":
        "https://github.com/palr0/nnotify/blob/main/image/gold.png?raw=true",
    "불안정한 제련소":
        "https://github.com/palr0/nnotify/blob/main/image/ref.png?raw=true",
    레이드: "https://github.com/palr0/nnotify/blob/main/image/raid.png?raw=true",
    "차원의 틈":
        "https://github.com/palr0/nnotify/blob/main/image/dimen.png?raw=true",
};

// 검증
if (!process.env.TOKEN) throw new Error("TOKEN 환경 변수가 필요합니다.");
if (!process.env.JSONBIN_API_KEY)
    throw new Error("JSONBIN_API_KEY 환경 변수가 필요합니다.");
if (!process.env.JSONBIN_BIN_ID)
    throw new Error("JSONBIN_BIN_ID 환경 변수가 필요합니다.");
...
async function saveMessageId(guildId, messageId) {
    try {
        ...
        console.log(
            `[${getKoreanTime()}] ✅ 메시지 ID 저장됨 (${guildId}): ${messageId}`,
        );
    } catch (err) {
        console.error(
            `[${getKoreanTime()}] ❌ 메시지 ID 저장 실패:`,
            err.message,
        );
    }
}
...
async function savePartyData(guildId) {
    try {
        ...
        console.log(
            `[${getKoreanTime()}] ✅ 파티 데이터 저장 완료 (${guildId})`,
        );
    } catch (err) {
        console.error(
            `[${getKoreanTime()}] ❌ 파티 데이터 저장 실패:`,
            err.message,
        );
    }
}
...
async function loadPartyData(guildId) {
    try {
        ...
        console.log(
            `[${getKoreanTime()}] ✅ 파티 데이터 로드 완료 (${guildId})`,
        );
    } catch (err) {
        console.error(
            `[${getKoreanTime()}] ❌ 파티 데이터 로드 실패:`,
            err.message,
        );
...
async function handleClearCommand(interaction) {
    ...
    const username =
        interaction.options.getString("닉네임") || interaction.user.username;
    ...
    if (command === "제거") {
        ...
        if (removed) {
            await interaction.reply({
                content: `${username} 님을 모든 클리어 목록에서 제거했습니다.`,
                ephemeral: true,
            });
        } else {
            await interaction.reply({
                content: `${username} 님은 클리어 목록에 없습니다.`,
                ephemeral: true,
            });
        }
    } else {
        if (!RAID_BOSSES.includes(bossName)) {
            await interaction.reply({
                content: "보스 이름은 '엑소' 또는 '테라'만 가능합니다.",
                ephemeral: true,
            });
            return;
        }
        ...
        if (!DIFFICULTIES.includes(difficulty)) {
            await interaction.reply({
                content: "난이도는 '노말', '하드', '노말하드'만 가능합니다.",
                ephemeral: true,
            });
            return;
        }
        ...
        await interaction.reply({
            content: `${username} 님이 ${bossName} ${difficulty} 클리어 목록에 추가되었습니다.`,
            ephemeral: true,
        });
    }
...
async function getSavedClearMessageId(guildId) {
    try {
        ...
        console.error(
            `[${getKoreanTime()}] ❌ 클리어 메시지 ID 불러오기 실패:`,
            err.message,
        );
        return null;
    }
...
async function saveClearMessageId(guildId, messageId) {
    try {
        ...
        console.log(
            `[${getKoreanTime()}] ✅ 클리어 메시지 ID 저장됨 (${guildId}): ${messageId}`,
        );
    } catch (err) {
        console.error(
            `[${getKoreanTime()}] ❌ 클리어 메시지 ID 저장 실패:`,
            err.message,
        );
    }
}
...
async function updateClearMessage(channel, guildId) {
    ...
    let clearMessage = messages.find(
        (m) => m.author.bot && m.content.includes("클리어명단"),
    );
    ...
    if (!clearMessage) {
        ...
        if (savedMessageId) {
            try {
                ...
                console.error(
                    `[${getKoreanTime()}] ❌ 저장된 클리어 메시지 불러오기 실패:`,
                    err.message,
                );
            }
        }
...
async function handlePartyCommand(interaction) {
    ...
    try {
        if (command === "생성") {
            ...
            if (guildParties[partyName])
                throw new Error("이미 존재하는 파티 제목입니다.");

            guildParties[partyName] = { members: new Set(), schedule: "" };
            await savePartyData(guildId);
            await interaction.reply({
                content: `파티 '${partyName}'가 생성되었습니다.`,
                ephemeral: true,
            });
            ...
            if (!guildParties[targetParty])
                throw new Error("존재하지 않는 파티 제목입니다.");
            if (!guildParties[targetParty].members.has(oldName)) {
                throw new Error(
                    `'${oldName}'님은 파티 '${targetParty}'에 존재하지 않습니다.`,
                );
            }
            ...
            await savePartyData(guildId);
            await interaction.reply({
                content: `파티 제목이 '${oldName}'에서 '${newName}'(으)로 변경되었습니다.`,
                ephemeral: true,
            });
            ...
            if (command === "등록") {
                if (position >= 0) {
                    const membersArray = Array.from(
                        guildParties[targetParty].members,
                    );
                ...
                await savePartyData(guildId);
                await interaction.reply({
                    content: `'${name}'님이 파티 '${targetParty}'에서 제거되었습니다.`,
                    ephemeral: true,
                });
            }
            ...
            if (!guildParties[partyToRemove])
                throw new Error("존재하지 않는 파티 제목입니다.");

            delete guildParties[partyToRemove];
            await savePartyData(guildId);
            await interaction.reply({
                content: `파티 '${partyToRemove}'가 삭제되었습니다.`,
                ephemeral: true,
            });
...
async function checkEmojiReactionsAndNotify(guild) {
    try {
        ...
        // 벨 이모지 반응 확인
        const bellReactions =
            targetMessage.reactions.cache.get(BOSS_ALERT_EMOJI);
        if (bellReactions) {
            const users = await bellReactions.users.fetch();
            const reactingUserIds = new Set(
            ...
            for (const [memberId, member] of role.members) {
                if (!reactingUserIds.has(memberId)) {
                    ...
                    console.log(
                        `[${getKoreanTime()}] 🔄 ${member.user.tag} 사용자가 이모지를 누르지 않았지만 역할이 남아있어 제거했습니다.`,
                    );
                }
            }
            ...
            for (const userId of reactingUserIds) {
                try {
                    ...
                    if (!member.roles.cache.has(role.id)) {
                        ...
                        console.log(
                            `[${getKoreanTime()}] ✅ ${member.user.tag} 사용자에게 알림 역할 부여 (이모지 등록 확인)`,
                        );
                    }
                } catch (err) {
                    console.error(
                        `[${getKoreanTime()}] ❌ ${userId} 사용자 역할 부여 실패:`,
                        err.message,
                    );
                }
            }
        ...
        if (dmReactions) {
            ...
            for (const userId of reactingUserIds) {
                ...
                console.log(
                    `[${getKoreanTime()}] ✉️ ${userId} 사용자가 DM 알림 등록`,
                );
            }
        }

        console.log(
            `[${getKoreanTime()}] 🔍 ${guild.name} 서버 이모지 상태 확인 완료: ${alertUsers.size}명 일반 알림, ${dmAlertUsers.size}명 DM 알림`,
        );
    } catch (err) {
        console.error(
            `[${getKoreanTime()}] ❌ 이모지 상태 확인 실패:`,
            err.message,
        );
    }
}
    ...
    const intervalId = setInterval(async () => {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setTitle("보스 알림 받기")
                .setDescription(
                    "새로운 보스 리젠 알림이 1분 전 올라옵니다! 알림을 받고 싶다면, 아래 이모지를 클릭해 주세요.",
                )
                .addFields(
                    {
                        name: "📢 다음 보스",
                        value: `**${nextBoss.boss}** (${nextBoss.timeStr})`,
                        inline: false,
                    },
                    {
                        name: "⏭️ 그 다음 보스",
                        value: `**${nextNextBoss.boss}** (${nextNextBoss.timeStr})`,
                        inline: false,
                    },
                    {
                        name: "🔔 일반 알림",
                        value: "이모지를 클릭하면 서버에서 멘션 알림을 받습니다.",
                        inline: true,
                    },
                    {
                        name: "📩 DM 알림",
                        value: "이모지를 클릭하면 개인 DM으로 알림을 받습니다.",
                        inline: true,
                    },
                )
                .setFooter({
                    text: `이모지를 클릭해서 원하는 알림을 받으세요!`,
                });
            ...
            if (timeUntilBoss <= 60000 && timeUntilBoss > 0) {
                const role = channel.guild.roles.cache.find(
                    (r) => r.name === ALERT_ROLE_NAME,
                );
                if (!role) {
                    console.error(
                        `[${getKoreanTime()}] ❌ ${ALERT_ROLE_NAME} 역할을 찾을 수 없습니다.`,
                    );
                    return;
                }
                ...
                if (membersWithRole === 0 && dmAlertUsers.size === 0) {
                    console.log(
                        `[${getKoreanTime()}] ⚠️ 알림을 받을 사용자가 없어 알림을 보내지 않습니다.`,
                    );
                    return;
                }
                ...
                if (
                    !bossMessages.has(
                        `${guildId}_alert_${nextBoss.boss}_${nextBoss.timeStr}`,
                    )
                ) {
                    // 일반 알림 (서버 멘션)
                    if (membersWithRole > 0) {
                        const mentions = Array.from(alertUsers)
                            .map((id) => `<@${id}>`)
                            .join(" ");
                        ...
                        const alertEmbed = new EmbedBuilder()
                            ...
                            .setTitle("⚠️ 보스 알림 ⚠️")
                            .setDescription(
                                `**${nextBoss.boss}**가 ${bossLocations[nextBoss.boss]}에 1분 후 출현합니다!`,
                            )
                            .addFields(
                                {
                                    name: "출현 시간",
                                    value: nextBoss.timeStr,
                                    inline: true,
                                },
                                {
                                    name: "알림",
                                    value: "이 알림은 1분 후에 자동으로 삭제됩니다.",
                                    inline: false,
                                },
                            )
                            .setFooter({
                                text: `출현 예정 시간: ${nextBoss.timeStr}`,
                            });
                        ...
                        console.log(
                            `[${getKoreanTime()}] 🔔 1분 전 알림 전송: ${nextBoss.boss} (${membersWithRole}명에게 전송)`,
                        );

                        setTimeout(() => {
                            alertMessage.delete().catch(console.error);
                            console.log(
                                `[${getKoreanTime()}] 🗑️ 보스 알림 메시지 삭제: ${nextBoss.boss}`,
                            );
                        }, 60000);
                    }
                    ...
                    if (dmAlertUsers.size > 0) {
                        for (const userId of dmAlertUsers) {
                            try {
                                ...
                                console.log(
                                    `[${getKoreanTime()}] ✉️ ${user.tag}에게 DM 알림 전송: ${nextBoss.boss}`,
                                );
                            } catch (dmErr) {
                                console.error(
                                    `[${getKoreanTime()}] ❌ ${userId} 사용자에게 DM 전송 실패:`,
                                    dmErr.message,
                                );
                            }
                        }
                        ...
                        setTimeout(
                            async () => {
                                ...
                                const cooldownAlertEmbed = new EmbedBuilder()
                                    ...
                                    .setTitle("🔄 보스 쿨타임 알림")
                                    .setDescription(
                                        `**${nextBoss.boss}** 보스 쿨타임이 돌아왔습니다. 확인해주세요!`,
                                    )
                                    .addFields(
                                        {
                                            name: "위치",
                                            value: bossLocations[nextBoss.boss],
                                            inline: true,
                                        },
                                        {
                                            name: "알림",
                                ...
                                console.log(
                                    `[${getKoreanTime()}] 🔄 ${nextBoss.boss} 쿨타임 알림 전송 완료`,
                                );
                            },
                            25 * 60 * 1000,
                        ); // 25분 후 알림
...
client.on("interactionCreate", async (interaction) => {
    ...
    try {
        // 클리어확인 채널 명령어 처리
        if (
            interaction.channel.name === CLEAR_CHANNEL_NAME &&
            interaction.commandName === "클"
        ) {
        ...
        // 파티명단 채널 명령어 처리
        if (
            interaction.channel.name === PARTY_CHANNEL_NAME &&
            interaction.commandName === "파티"
        ) {
        ...
        if (interaction.channel.name !== BOSS_CHANNEL_NAME) {
            await interaction.reply({
                content: "⚠️ 이 명령어는 #보스알림 채널에서만 사용 가능합니다.",
                ephemeral: true,
            });
            return;
        }
        ...
        if (interaction.commandName === "알림초기화") {
            ...
            // 기존 봇 메시지 일괄 삭제
            const messages = await interaction.channel.messages.fetch();
            const deletionPromises = messages
                .filter((m) => m.author.bot && m.id !== interaction.id)
                .map((msg) =>
                    msg
                        .delete()
                        .catch((e) =>
                            console.error(
                                `[${getKoreanTime()}] 메시지 삭제 실패: ${e.message}`,
                            ),
                        ),
                );
...
client.on("messageReactionAdd", async (reaction, user) => {
    ...
    try {
        ...
        if (reaction.emoji.name === BOSS_ALERT_EMOJI) {
            ...
            console.log(
                `[${getKoreanTime()}] ✅ ${user.tag} 알림 등록 및 역할 부여`,
            );
        }
        // DM 이모지 처리
            ...
            try {
                const dmEmbed = new EmbedBuilder()
                    ...
                    .setDescription(
                        `보스 출현 1분 전에 DM으로 알림을 보내드립니다.\n\n서버: ${reaction.message.guild.name}`,
                    )
                    .setFooter({
                        text: "알림을 취소하려면 이모지를 다시 클릭해주세요.",
                    });

                await user.send({ embeds: [dmEmbed] });
            } catch (dmErr) {
                console.error(
                    `[${getKoreanTime()}] ❌ ${user.tag}에게 DM 전송 실패:`,
                    dmErr.message,
                );
            }
        }
    ...
    try {
        ...
        if (reaction.emoji.name === BOSS_ALERT_EMOJI) {
            ...
            if (role && member.roles.cache.has(role.id)) {
                ...
                console.log(
                    `[${getKoreanTime()}] 🔕 ${user.tag} 알림 해제 및 역할 제거`,
                );
                ...
                if (reactions) {
                    ...
                    if (!users.has(user.id)) {
                        await member.roles.remove(role).catch(console.error);
                        console.log(
                            `[${getKoreanTime()}] 🔄 ${user.tag} 사용자가 이모지를 누르지 않았지만 역할이 남아있어 제거했습니다.`,
                        );
                    }
                }
            }
        ...
        // DM 이모지 처리
            ...
            try {
                const dmEmbed = new EmbedBuilder()
                    ...
                    .setDescription(
                        `더 이상 보스 출현 DM 알림을 받지 않습니다.\n\n서버: ${reaction.message.guild.name}`,
                    )
                    .setFooter({
                        text: "다시 등록하려면 이모지를 클릭해주세요.",
                    });
...
async function resetAllClearData() {
    clearData.forEach((guildData, guildId) => {
        ...
        if (guild) {
            const clearChannel = guild.channels.cache.find(
                (c) => c.name === CLEAR_CHANNEL_NAME,
            );
            if (clearChannel) {
                // 기본 형태로 메시지 업데이트
                    ...
                    .then((messages) => {
                        ...
                        if (clearMessage) {
                            ...
                            clearMessage
                                ...
                                .then(() =>
                                    console.log(
                                        `[${getKoreanTime()}] ✅ ${guild.name} 서버 클리어 명단 초기화 완료`,
                                    ),
                                )
                                .catch((err) =>
                                    console.error(
                                        `[${getKoreanTime()}] ❌ 메시지 수정 실패:`,
                                        err,
                                    ),
                                );
                        } else {
                            ...
                            clearChannel
                                .send(defaultMessage)
                                .then((msg) =>
                                    saveClearMessageId(guildId, msg.id),
                                )
                                .then(() =>
                                    console.log(
                                        `[${getKoreanTime()}] ✅ ${guild.name} 서버 클리어 명단 초기화 완료`,
                ...
                clearChannel
                    .send(
                        "🔄 **클리어 명단이 주간 초기화되었습니다!** 새로운 주도 화이팅! 💪",
                    )
                    .then((msg) => {
                        console.log(
                            `[${getKoreanTime()}] ⏳ 초기화 알림 메시지 전송 (30분 후 삭제 예정)`,
                        );
                        setTimeout(
                            () => {
                                msg.delete()
                                    .then(() =>
                                        console.log(
                                            `[${getKoreanTime()}] 🗑️ 초기화 알림 메시지 삭제 완료`,
                                        ),
                                    )
                                    .catch((err) =>
                                        console.error(
                                            `[${getKoreanTime()}] ❌ 알림 메시지 삭제 실패:`,
                                            err,
                                        ),
                                    );
                            },
    ...
    console.log(
        `[${getKoreanTime()}] 🔄 모든 서버 클리어 데이터 주간 초기화 완료`,
    );
}
...
client.once("ready", async () => {
    ...
    console.log(
        `[${getKoreanTime()}] ✅ ${client.user.tag} 봇이 온라인입니다!`,
    );
    console.log(
        `[${getKoreanTime()}] 🟢 봇 시작 - ${new Date().toISOString()}`,
    );
        ...
        for (const [guildId, guild] of client.guilds.cache) {
            try {
                ...
                if (!role) {
                    ...
                    console.log(
                        `[${getKoreanTime()}] ✅ ${guild.name} 서버에 ${ALERT_ROLE_NAME} 역할 생성 완료`,
                    );
                } else {
                    ...
                    if (membersWithRole.size > 0) {
                        ...
                        console.log(
                            `[${getKoreanTime()}] 🔄 ${guild.name} 서버의 기존 ${ALERT_ROLE_NAME} 역할 보유자 ${membersWithRole.size}명에서 역할 제거 완료`,
                        );
                    }
                }

                // 보스알림 채널 설정
                const bossAlertChannel = guild.channels.cache.find(
                    (c) =>
                ...
                if (bossAlertChannel) {
                    ...
                    if (savedMessageId) {
                        try {
                            bossMessage =
                                await bossAlertChannel.messages.fetch(
                                    savedMessageId,
                                );
                            const reactions =
                                bossMessage.reactions.cache.get(
                                    BOSS_ALERT_EMOJI,
                                );
                            if (reactions) {
                                const users = await reactions.users.fetch();
                                    ...
                                    users
                                        ...
                                        .map(async (user) => {
                                            try {
                                                ...
                                                alertUsers.add(user.id);
                                                console.log(
                                                    `[${getKoreanTime()}] ✅ ${user.tag} 기존 알림 등록자 역할 자동 부여`,
                                                );
                                            } catch (err) {
                                                console.error(
                                                    `[${getKoreanTime()}] ❌ ${user.tag} 역할 부여 실패:`,
                                                    err.message,
                                                );
                                            }
                                        }),
                            ...
                            bossMessages.set(guildId, bossMessage);
                            console.log(
                                `[${getKoreanTime()}] ✅ ${guild.name} 서버 이전 메시지 불러오기 성공: ${bossMessage.id}`,
                            );
                        } catch (fetchErr) {
                            console.error(
                                `[${getKoreanTime()}] ⚠️ ${guild.name} 서버에서 메시지 불러오기 실패:`,
                                fetchErr.message,
                            );
                        }
                    }
                    ...
                    if (!bossMessage) {
                        ...
                        bossMessage = await bossAlertChannel.send({
                            embeds: [embed],
                        });
                        await bossMessage.react(BOSS_ALERT_EMOJI);
                        await bossMessage.react(DM_ALERT_EMOJI);
                        ...
                        await saveMessageId(guildId, bossMessage.id);
                        console.log(
                            `[${getKoreanTime()}] ✅ ${guild.name} 서버에 새 메시지 생성: ${bossMessage.id}`,
                        );
                    }
                ...
                // 파티 채널 초기화
                const partyChannel = guild.channels.cache.find(
                    (c) => c.name === PARTY_CHANNEL_NAME,
                );
                if (partyChannel) {
                ...
                console.error(
                    `[${getKoreanTime()}] ❌ ${guild.name} 서버 초기화 실패:`,
                    guildErr.message,
                );
            }
        }
...
function getTodayDungeon() {
    ...
    if ([2, 4, 6].includes(day)) {
        dungeons.push({
            title: "불안정한 제련소",
            description:
                "몬스터와 맞서 싸우고 미가공 강화 원석(정교한 강화석, 경험치)을 쟁취하세요!",
    ...
    if (day === 4) {
        dungeons.push({
            title: "레이드",
            description:
                "강력한 레이드 보스와의 전투에서 승리하여 전리품을 획득하세요!",
    ...
    if (day === 0) {
        dungeons.push({
            title: "차원의 틈",
            description:
                "몬스터와 맞서 싸우고 디멘션 조각(열쇠, 경험치)을 쟁취하세요!",
...
async function sendDailyDungeonMessage() {
    ...
    for (const [guildId, guild] of client.guilds.cache) {
        try {
            const dungeonChannel = guild.channels.cache.find(
                (c) =>
            ...
            console.log(
                `[${getKoreanTime()}] ✅ ${guild.name} 서버에 오늘의 던전 메시지 전송 완료`,
            );
        } catch (err) {
            console.error(
                `[${getKoreanTime()}] ❌ ${guild.name} 서버 던전 메시지 전송 실패:`,
                err.message,
            );
        }
    }
...
function setupDailyDungeonSchedule() {
    ...
    console.log(
        `[${getKoreanTime()}] ⏰ 오늘의 던전 스케줄러 설정 완료 (${midnight.toLocaleString("ko-KR")} 실행 예정)`,
    );
}
